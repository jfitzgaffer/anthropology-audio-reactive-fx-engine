...
class TitanQtGUI(QObject):
    ...
    def refresh_logic(self):
        ...
        if hasattr(self, 'curve_audio'):
            self.curve_audio.setData(list(self.engine.scope_audio))
            self.curve_center.setData(list(self.engine.scope_center))
            self.curve_edge.setData(list(self.engine.scope_edge))

            if hasattr(self, 'dmx_boxes'):
                mon_univ = self.spin_mon_univ.value()
                current_univ_data = self.engine.dmx_buffers.get(mon_univ, [0] * 512)
