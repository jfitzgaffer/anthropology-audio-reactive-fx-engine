...
* **Main Branch Default — No Worktrees Unless Asked:** All code edits MUST land directly in the `main` branch of the primary working directory (`/Users/jefffitzgerald/Anthropology/Anthropology Lighting Effects Engine_v3`). If you are invoked inside a Claude worktree (`.claude/worktrees/<name>/`) on a side branch like `claude/<name>`, do NOT start editing there. Switch your working directory back to the main repo root first, confirm you are on `main`, and make the changes on `main`. The only exception is when I explicitly say "use a worktree for this" or "do this in a separate branch". Worktrees caused a confusing round-trip (commit in worktree → merge into main → resolve conflicts → PyCharm still blind to changes) that wasted time on 2026-04-20; the default is always `main` in the primary tree.
